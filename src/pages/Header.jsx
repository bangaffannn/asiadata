...
import logo from "../assets/logoasiadata.png";
...
function Header() {
  ...
  return (
    <header className="p-8 md:p-0 text-white bg-black">
      <nav className="container mx-auto">
        <button className="md:hidden z-40 text-2xl w-full flex justify-end md:justify-center items-center" onClick={toggleMenu}>
          {isMenuOpen ? <AiOutlineClose /> : <RiMenu3Fill />}
        </button>
        <div className="flex justify-between items-center w-full">
          <img className="hidden md:block h-36 w-auto" src={logo} alt="logo asiadata" />
          <div>
            <ul className={`${
              isMenuOpen
                ? "fixed md:hidden mx-4 my-6 top-10 left-0 right-0 bg-opacity-80 flex flex-col"
